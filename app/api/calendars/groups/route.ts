...
import {eventFetchQueue, notificationQueue} from "@/lib/queues";
...
export async function POST(req: NextRequest){
    try {
        ...
        await eventFetchQueue.add('fetch-events',{provider: dbCalendar.provider, providerCalendarId: dbCalendar.providerCalendarId, userId: user.id, calendarId: dbCalendar.id}, {
            attempts: 5,
            backoff: {
                type: "exponential",
                delay: 5000
        ...
        await notificationQueue.add('group-created-notification',{userId: user.id,
